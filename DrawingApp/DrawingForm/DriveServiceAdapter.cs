...
namespace DrawingForm
{
    public class DriveServiceAdapter : IDriveService
    {
        ...
        public DriveServiceAdapter()
        {
            ...
            _clientSecretFileName = CLIENT_SECRET_FILE_NAME;
            this.CreateNewService(APPLICATION_NAME, CLIENT_SECRET_FILE_NAME);
        }
        ...
        public DriveServiceAdapter(string applicationName, string clientSecretFileName)
        {
        ...
        private async void CreateNewService(string applicationName, string clientSecretFileName)
        {
            ...
            using (FileStream stream = new FileStream(clientSecretFileName, FileMode.Open, FileAccess.Read))
            {
                ...
                credential = await Task.Factory.StartNew(() => GoogleWebAuthorizationBroker.AuthorizeAsync(GoogleClientSecrets.Load(stream).Secrets, _scopes, USER, CancellationToken.None, new FileDataStore(credentialPath, true)).Result);
            }
